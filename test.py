...
def recommendation(
    ...
    with open("Fertclassifier.pkl", "rb") as f:
        model = pickle.load(f)
    predictions = model.predict_proba(features)[0]  # Get the probability distribution
    top_indices = np.argsort(predictions)[::-1][:num_recommendations]  # Get top indices
    recommended_fertilizers = [
        y.unique()[i] for i in top_indices
...
recommendations = recommendation(
    ...
    soil_type,
    crop_type,
    num_recommendations=5,  # Get top 5 recommendations
)
print(f"Fertilizer Recommendations: {recommendations}")
